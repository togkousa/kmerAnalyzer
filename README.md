...
### Extra comments
- It's important to have a look at the lengths of the sequences, prior to executing kmerAnalyzer. For example, if the sequence with header `>ERR525627.984.1 984 length=31` in `data/sra_data.fasta` has length 31, so probably its better to examine lower k-values, e.g. up to 20.
